...
public class BinaryTree<E> {
	...
	public BinaryTree<E> copyToDepth(int d) {
		...
		if (d > 0) { //if we have to keep going down the tree,
			if (hasLeft()) { //if there is a left node
				copy.left = this.left.copyToDepth(d-1); //do the left node
			}
			if (hasRight()) {//if there is a right
				copy.right = this.right.copyToDepth(d-1); //do the right
			}
		}
		else { //if we stop here,
			copy.left = null; //don't copy left
			copy.right = null; //or right
		}
